...
async function loadBudgetForMonth(sheetIndex) {
  ...
  console.log('[loadBudgetForMonth] sheetIndex =', sheetIndex, 'API response keys:', Object.keys(data || {}));
  ...
  if (data && typeof data === 'object') {
    Object.keys(data).forEach(key => {
      ...
      console.log('[loadBudgetForMonth] key =', key, 'isExpenseBudget =', isExpenseBudget, 'rows.length =', rows.length);
      if (!isExpenseBudget) {
        return;
      }
      rows.forEach((row, rowIndex) => {
        ...
        if (rowIndex < 3) {
          console.log('[loadBudgetForMonth] row[' + rowIndex + ']:', 'category=', category, 'costRaw=', costRaw, 'cost=', cost);
        }
    ...
    console.warn('[loadBudgetForMonth] Invalid data format:', data);
  }

  budgetTotals[sheetIndex] = categoryTotals;
  console.log('[loadBudgetForMonth] categoryTotals:', categoryTotals);
...
const updateTotalDisplay = () => {
  ...
  console.log('[支出表] updateTotalDisplay: currentSheetIndex =', currentSheetIndex, 'currentCategory =', currentCategory);

  // 1. 預算：從預算表預先彙總好的資料中讀出（快取在 budgetTotals）
  let budget = 0;
  const budgetData = budgetTotals[currentSheetIndex];
  console.log('[支出表] updateTotalDisplay: budgetData =', budgetData);
  if (budgetData && currentCategory) {
    budget = parseFloat(budgetData[currentCategory]) || 0;
  }
  console.log('[支出表] updateTotalDisplay: budget =', budget);

  // 2. 支出：從當前月份的回傳資料中，篩出同類別的，並加上即時輸入金額
  let historyExpense = 0;
  let records = [];
  const monthData = allMonthsData[currentSheetIndex];
  console.log('[支出表] updateTotalDisplay: monthData =', monthData ? 'exists' : 'undefined', 'monthData.data =', monthData?.data ? 'exists' : 'undefined');
  if (monthData && monthData.data) {
    // 只有在有暫存資料時才從暫存載入，避免一開始還沒載完就一直噴警告
    records = loadHistoryListFromCache(currentSheetIndex); // 只讀暫存，不發請求
  }
  console.log('[支出表] updateTotalDisplay: records.length =', records.length);
...
const showSpinner = () => {
  ...
  // 如果歷史紀錄模態框打開，spinner 的 z-index 要高於模態框（10000）
  ...
  spinnerContainer.innerHTML = `
    <div style="text-align: center;">
      <i class="fas fa-spinner fa-spin" style="font-size: 48px; color: #3498db; margin-bottom: 20px;"></i>
      <div class="spinner-text" style="font-size: 16px; color: #666;">載入中...</div>
    </div>
  `;
...
const hideSpinner = () => {
  ...
  if (overlay) {
    ...
    if (spinnerContainer) {
      const spinnerIcon = spinnerContainer.querySelector('.fa-spinner');
      const spinnerText = spinnerContainer.querySelector('.spinner-text');
      if (spinnerIcon) {
        spinnerIcon.className = 'fas fa-check';
        spinnerIcon.style.color = '#2ecc71';
      }
      if (spinnerText) {
        spinnerText.textContent = '✓ 已完成';
        spinnerText.style.color = '#2ecc71';
...
async function showHistoryModal() {
  ...
  // 創建載入中顯示（使用 Font Awesome spinner）
  ...
  try {
    ...
    try {
      ...
      if (sheetNames.length > 0) {
        ...
        // 先載入當前月份的預算資料，再更新總計顯示
        try {
          console.log('[支出表] 載入當月預算資料...', currentSheetIndex);
          await loadBudgetForMonth(currentSheetIndex);
          console.log('[支出表] 當月預算資料載入完成, budgetTotals =', budgetTotals[currentSheetIndex]);
        } catch (budgetErr) {
          console.error('[支出表] 載入當月預算資料失敗:', budgetErr);
        }
        ...
        sheetNames.forEach((name, idx) => {
          const sheetIndex = idx + 2;
          // 跳過當前月份（已經載入）
          if (sheetIndex === currentSheetIndex) return;
          loadBudgetForMonth(sheetIndex)
            .catch(err => {
              console.error('[支出表] 載入預算資料失敗 (sheetIndex=' + sheetIndex + '):', err);
            });
        });
      }
    } catch (e) {
