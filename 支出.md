...
<link rel="stylesheet" href="{{ '/assets/common.css' | relative_url }}">
...
// Create placeholder for sticky summary (prevents content jump)
const totalContainerPlaceholder = document.createElement('div');
totalContainerPlaceholder.className = 'total-container-placeholder';
...
// Sticky summary scroll handler (mobile only)
  ...
  const updateStickyState = () => {
    if (!isMobile()) {
      ...
      if (isSticky) {
        ...
        totalContainerPlaceholder.classList.remove('visible');
        isSticky = false;
      }
      return;
    }

    // Get the original position of totalContainer
    if (!isSticky) {
    ...
    const navHeight = 56; // Height of nav bar
    const scrollY = window.pageYOffset || document.documentElement.scrollTop;
    const shouldBeSticky = scrollY > (originalTop - navHeight);

    if (shouldBeSticky && !isSticky) {
      // Insert placeholder before totalContainer
      totalContainer.parentNode.insertBefore(totalContainerPlaceholder, totalContainer);
      totalContainerPlaceholder.style.height = totalContainer.offsetHeight + 'px';
      totalContainerPlaceholder.classList.add('visible');
