  ...
  if (data && typeof data === 'object') {
    ...
    Object.keys(data).forEach(key => {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        // 跳過標題列與總計列
        if (firstCell === '編號') {
          console.log('[支出表] loadBudgetForMonth: 跳過標題列', {
            key: key,
            rowIndex: rowIndex,
            row: row
          });
        ...
        const item = (row[3] || '').toString().trim(); // item 在第 3 欄（索引 3）
        ...
        // 重要：如果有兩個同項目的預算要相加，而不是取第一筆
        // 使用 category + item 作為 key，確保相同項目的預算會相加
        const budgetKey = item ? `${category}_${item}` : category;
        const oldTotal = categoryTotals[budgetKey] || 0;
        categoryTotals[budgetKey] = oldTotal + cost;

        console.log('[支出表] loadBudgetForMonth: 更新預算總計', {
          budgetKey: budgetKey,
          category: category,
          item: item,
          cost: cost,
    ...
    console.log('[支出表] loadBudgetForMonth: 處理資料完成', {
