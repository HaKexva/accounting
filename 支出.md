...
// 重新計算上方「預算 / 支出 / 餘額」
// 預算：從預算表的 total 欄位讀取（所有類別加總）
// 支出：從當前月份的所有歷史記錄加總（不分類別）
const updateTotalDisplay = () => {
  const recordCostInput = document.getElementById('record-cost-input');

  // 1. 預算：從預算表的 budgetTotals 加總所有類別
  let budget = 0;
  const budgetData = budgetTotals[currentSheetIndex];
  if (budgetData && typeof budgetData === 'object') {
    budget = Object.values(budgetData).reduce((sum, val) => sum + (parseFloat(val) || 0), 0);
  }

  // 2. 支出：從當前月份的所有歷史記錄加總（不分類別）
...
async function showHistoryModal() {
  ...
  // 先檢查是否有快取資料（記憶體或 sessionStorage）
