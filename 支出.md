...
if (recordCostInput) {
  ...
  // Sticky summary on mobile when record-cost input is focused
  ...
  const updateStickyPosition = () => {
    ...
    // Use visual viewport offset if available (handles keyboard)
    const vv = window.visualViewport;
    if (vv) {
      const topOffset = vv.offsetTop + 62; // 62px below nav bar
      totalContainer.style.top = topOffset + 'px';
    }
  };
  ...
  recordCostInput.addEventListener('focus', () => {
    if (isMobile() && totalContainer && totalContainer.parentNode) {
      ...
      // Set placeholder height and make sticky
  ...
  recordCostInput.addEventListener('blur', () => {
    if (totalContainer) {
      ...
      totalContainer.style.top = ''; // Reset to CSS default
