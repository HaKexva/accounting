...
// 顯示快取時間戳記
const showCacheTimestamp = (timestamp) => {
  let timestampEl = document.getElementById('cache-timestamp');
  if (!timestampEl) {
    timestampEl = document.createElement('div');
    timestampEl.id = 'cache-timestamp';
  ...
  try {
    ...
    if (loadedFromCache) {
      try {
        ...
        // 顯示快取時間戳
        if (cachedTimestamp) {
          showCacheTimestamp(cachedTimestamp);
        }

        // 確保 DOM 完全渲染後再隱藏 spinner
        ...
        console.error('[支出表] 快取載入失敗，改用 API:', cacheError);
