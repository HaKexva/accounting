...
const baseBudget  = "https://script.google.com/macros/s/AKfycbwrH_boSxuX6IDp1Pw2s7opjZiYqTKFrvP-KqNXLKEDZLPOzp4mzRt8XZZgvR-OcZqN/exec";
  ...
  try {
    ...
    if (result && result.data) {
      ...
      if (newRecords && newRecords.length > 0) {
        ...
        const lastRecordNumber = parseInt(lastRecord.row[0], 10);

        // 顯示這筆記錄
        if (Number.isFinite(lastRecordNumber) && lastRecordNumber > 0) {
          // 設置記錄編號顯示
          const recordNumberEl = document.getElementById('record-number');
          if (recordNumberEl) {
            recordNumberEl.textContent = lastRecordNumber;
            recordNumberEl.style.display = 'block';
          }
          ...
          // 支出格式：[時間(0), item(1), category(2), spendWay(3), creditCard(4), monthIndex(5), actualCost(6), payment(7), recordCost(8), note(9)]
          if (itemInput) itemInput.value = lastRecord.row[1] || '';
          if (expenseCategorySelect) {
            expenseCategorySelect.value = lastRecord.row[2] || '';
            // 更新自訂下拉顯示
            const categoryContainer = expenseCategorySelect.parentElement;
            if (categoryContainer) {
              const categoryDisplay = categoryContainer.querySelector('.select-display');
              if (categoryDisplay) {
                const categoryText = categoryDisplay.querySelector('.select-text');
                if (categoryText) {
                  categoryText.textContent = lastRecord.row[2] || '';
                }
              }
            }
          }
          if (paymentMethodSelect) {
            paymentMethodSelect.value = lastRecord.row[3] || '';
            // 更新自訂下拉顯示
            const paymentContainer = paymentMethodSelect.parentElement;
            if (paymentContainer) {
              const paymentDisplay = paymentContainer.querySelector('.select-display');
              if (paymentDisplay) {
                const paymentText = paymentDisplay.querySelector('.select-text');
                if (paymentText) {
                  paymentText.textContent = lastRecord.row[3] || '';
                }
              }
            }
          }
          if (creditCardPaymentSelect && lastRecord.row[4]) {
            creditCardPaymentSelect.value = lastRecord.row[4] || '';
          }
          if (monthPaymentSelect && lastRecord.row[5]) {
            monthPaymentSelect.value = lastRecord.row[5] || '';
          }
          if (actualCostInput) actualCostInput.value = lastRecord.row[6] || '';
          if (paymentPlatformSelect && lastRecord.row[7]) {
            paymentPlatformSelect.value = lastRecord.row[7] || '';
          }
          if (recordCostInput) recordCostInput.value = lastRecord.row[8] || '';
          if (noteInput) noteInput.value = lastRecord.row[9] || '';
          if (recordDate) recordDate.textContent = formatRecordDateTime(lastRecord.row[0] || '');
        }
      }
    }

    // 清空表單（但保留剛顯示的記錄）
    // clearForm(); // 不再清空，因為已經顯示了剛新增的記錄
  } catch (error) {
