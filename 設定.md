...
function initDragAndDrop(list, itemId) {
  ...
  // 禁用列表滾動和其他交互的函數（只在真正拖曳時使用）
  function disableInteractions() {
    // 只禁用列表內其他選項項的點擊，不影響列表滾動
  ...
  // 只在拖拽手柄上绑定事件（同時支援桌面和手機）
  list.querySelectorAll('.drag-handle').forEach((handle) => {
    // 桌面版：HTML5 Drag and Drop
    ...
    handle.addEventListener('touchstart', (e) => {
      ...
      // 先不設置樣式，等確認真的在拖曳時再設置
      window._isDragging = false; // 先不標記為拖曳，等確認移動後再標記
    }, { passive: true }); // 使用 passive: true，不阻止默認行為
    ...
    handle.addEventListener('touchmove', (e) => {
      ...
      // 檢查是否有明顯的移動（避免誤觸）
      // 如果滾動距離大於垂直移動，說明是在滾動，不處理拖曳
      if (scrollDelta > Math.abs(deltaY) * 0.5) {
        // 正在滾動，不處理拖曳
        return;
      }
      ...
      if (Math.abs(deltaY) > 10) {
        if (!hasMoved) {
          ...
          window._isDragging = true;
          disableInteractions(); // 只在確認拖曳時才禁用交互
        }
        ...
        if (afterElement == null) {
          ...
          // 插入到指定元素之前（移動到第一筆時，afterElement 應該是第一個元素）
...
function getDragAfterElement(container, y, draggedElement) {
  ...
  return draggableElements.reduce((closest, child) => {
    ...
    // 如果游標在上半部分（offset < 0），且比之前的最近元素更接近，則更新
...
// 防止 iOS Safari 回彈效果（兩個方向都不要），但不影響拖曳和正常滾動
(function() {
  let startY = 0;
  ...
  let startScrollContainer = null; // 記錄開始觸摸時的滾動容器
  
  // 鎖定滾動位置
  function lockScrollPosition(container, position) {
    if (scrollLockTimer) {
      clearInterval(scrollLockTimer);
      cancelAnimationFrame(scrollLockTimer);
    }
    lockedScrollTop = position;
    const containerRef = container;
    
    function forceLock() {
      if (lockedScrollTop !== null && !window._isDragging) {
        if (containerRef === document.documentElement) {
          window.scrollTo(0, lockedScrollTop);
          document.documentElement.scrollTop = lockedScrollTop;
          document.body.scrollTop = lockedScrollTop;
        } else {
          containerRef.scrollTop = lockedScrollTop;
        }
        scrollLockTimer = requestAnimationFrame(forceLock);
      }
    }
    
    // 使用 requestAnimationFrame 更頻繁地鎖定
    scrollLockTimer = requestAnimationFrame(forceLock);
  }
  ...
  document.addEventListener('touchstart', function(e) {
    ...
    unlockScrollPosition(); // 先解鎖
    
    startY = e.touches[0].clientY;
    // 檢查是否在下拉選單容器內
    const dropdownContainer = e.target.closest('.category-dropdown, .select-dropdown, .options-list, .select-options');
    if (dropdownContainer) {
      startScrollContainer = dropdownContainer;
      startScrollTop = dropdownContainer.scrollTop;
    } else {
      startScrollContainer = document.documentElement;
  ...
  document.addEventListener('touchmove', function(e) {
    // 如果正在拖曳（HTML5 Drag），完全不處理回彈防止，允許正常移動
    if (window._isDragging) {
      unlockScrollPosition();
      return; // 不阻止，讓拖曳正常進行
    }
    
    // 如果是從拖拽手柄開始的觸摸，允許正常移動
    if (touchStartTarget && isDragHandle(touchStartTarget)) {
      unlockScrollPosition();
      return; // 不阻止，允許上下移動
    }
    
    if (!canPrevent || !startScrollContainer) {
      unlockScrollPosition();
      return;
    }
    ...
    // 在邊界時，更積極地阻止回彈並立即強制設置位置
    if (e.cancelable) {
      let shouldPrevent = false;
      let targetScroll = null;
      
      // 在頂部，如果向下拉（deltaY > 0），阻止並鎖定位置
      if (isAtTop && deltaY > 0) {
        shouldPrevent = true;
        targetScroll = 0;
      }
      // 在底部，如果向上拉（deltaY < 0），阻止並鎖定位置
      else if (isAtBottom && deltaY < 0) {
        shouldPrevent = true;
        targetScroll = scrollHeight - clientHeight;
      }
      // 如果從邊界開始且繼續向邊界外移動，也阻止
      else {
        const wasAtTop = startScrollTop <= 0;
        const wasAtBottom = startScrollTop + clientHeight >= scrollHeight - 1;
        
        if (wasAtTop && deltaY < 0) {
          // 從頂部開始，向上移動
          shouldPrevent = true;
          targetScroll = 0;
        } else if (wasAtBottom && deltaY > 0) {
          // 從底部開始，向下移動
          shouldPrevent = true;
          targetScroll = scrollHeight - clientHeight;
        }
      ...
      if (shouldPrevent && targetScroll !== null) {
        ...
        if (startScrollContainer === document.documentElement) {
          ...
          startScrollContainer.scrollTop = targetScroll;
        }
        lockScrollPosition(startScrollContainer, targetScroll);
        canPrevent = false;
        return false;
      }
    }
    
    unlockScrollPosition(); // 如果沒有阻止，解鎖
  }, { passive: false });
  
  document.addEventListener('touchend', function() {
    touchStartTarget = null;
    startScrollContainer = null;
    unlockScrollPosition(); // 解鎖
    // 如果不在拖曳狀態，重置
    if (!window._isDragging) {
      canPrevent = false;
    }
