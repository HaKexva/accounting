...
(function() {
  ...
  document.addEventListener('touchmove', function(e) {
    ...
    let scrollContainer, currentScrollTop, scrollHeight, clientHeight;
    
    if (dropdownContainer) {
      // 在下拉選單容器內，檢查容器的滾動邊界
      scrollContainer = dropdownContainer;
      currentScrollTop = dropdownContainer.scrollTop;
      scrollHeight = dropdownContainer.scrollHeight;
      clientHeight = dropdownContainer.clientHeight;
    } else {
      // 在頁面主體，檢查頁面的滾動邊界
    ...
    // 檢查是否在邊界
    const isAtTop = currentScrollTop <= 1; // 允許1px誤差
    ...
    if (e.cancelable) {
      if (isAtTop && deltaY > 0) {
        ...
        e.preventDefault();
        canPrevent = false;
      } else if (isAtTop && (dropdownContainer ? dropdownContainer.scrollTop <= 1 : startScrollTop <= 1) && deltaY < 0) {
        // 在頂部，向上滑（回彈）
        e.preventDefault();
        canPrevent = false;
      } else if (isAtBottom && (dropdownContainer ? dropdownContainer.scrollTop + dropdownContainer.clientHeight >= dropdownContainer.scrollHeight - 1 : startScrollTop + clientHeight >= scrollHeight - 1) && deltaY > 0) {
        // 在底部，向下滑（回彈）
        e.preventDefault();
        canPrevent = false;
      }
