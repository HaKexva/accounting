...
function createPanel(item) {
  ...
  panel.innerHTML = `
    ...
    <div class="panel-content">
      <div class="reorder-toggle-container">
        <label class="reorder-toggle-label">
          <input type="checkbox" id="reorder-mode-${item.id}" class="reorder-mode-checkbox">
          <span>更改支出排序</span>
        </label>
      </div>
...
function initDragAndDrop(list, itemId) {
  ...
  list.querySelectorAll('.drag-handle').forEach((handle) => {
    ...
    // 手機版：觸摸拖曳（只有三條橫線可以觸發，且需啟用排序模式）
    handle.addEventListener('touchstart', (e) => {
      // 檢查是否啟用排序模式
      const reorderCheckbox = document.getElementById(`reorder-mode-${itemId}`);
...
(function() {
  ...
  document.addEventListener('touchstart', function(e) {
    ...
    if (isDragHandle(e.target) || window._isDragging) {
      canPrevent = false;
      return;
    }
  ...
  document.addEventListener('touchmove', function(e) {
    if (window._isDragging || (touchStartTarget && isDragHandle(touchStartTarget))) {
      return;
    }
  ...
  function setupDropdownPrevention() {
    ...
    dropdowns.forEach(dropdown => {
      ...
      dropdown.addEventListener('touchmove', function(e) {
        if (window._isDragging || isDragHandle(e.target)) {
          return;
        }
