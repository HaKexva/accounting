  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      if (result.data && result.total) {
        ...
        // 如果是修改模式，直接更新對應的記錄，避免 processDataFromResponse 清空 allRecords 導致重複
        if (!wasInNewMode && savedRecordNumber !== null) {
          // 修改模式：從 result.data 中找到對應的記錄並更新
          const updatedRecord = findRecordInData(result.data, savedRecordNumber, savedType);
          if (updatedRecord) {
            // 找到 allRecords 中對應的記錄並更新
            const recordIndex = allRecords.findIndex(r => {
              const num = parseInt(r.row[0], 10);
              return Number.isFinite(num) && num > 0 && num === savedRecordNumber && r.type === savedType;
            });
            if (recordIndex >= 0) {
              console.log('[預算表] saveData: 更新現有記錄', {
                recordNumber: savedRecordNumber,
                recordIndex: recordIndex,
                oldRecord: allRecords[recordIndex],
                newRecord: updatedRecord
              });
              allRecords[recordIndex] = updatedRecord;
            } else {
              // 如果找不到，可能是記錄被刪除了，重新處理所有數據
              console.warn('[預算表] saveData: 找不到要更新的記錄，重新處理所有數據', {
                recordNumber: savedRecordNumber,
                savedType: savedType
              });
            ...
            // 如果找不到更新的記錄，重新處理所有數據
            console.warn('[預算表] saveData: 在回傳資料中找不到更新的記錄，重新處理所有數據', {
              recordNumber: savedRecordNumber,
              savedType: savedType
            });
            processDataFromResponse(result.data, false);
        ...
        if (result.total && Array.isArray(result.total) && result.total.length >= 3) {
          console.log('[預算表] saveData: 使用後端返回的總計', {
            total: result.total,
            income: result.total[0],
            expense: result.total[1],
            totalAmount: result.total[2]
          });
