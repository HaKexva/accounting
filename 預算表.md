...
const loadMonthData = async (sheetIndex, useGlobalAbortController = true) => {
  ...
  if (!totalData || !Array.isArray(totalData) || totalData.length < 3) {
    ...
    // 驗證總計值是否合理（如果 API 返回的支出為 0 但我們計算出有支出，使用計算的值）
    const apiIncome = parseFloat(totalData[0]) || 0;
    const apiExpense = parseFloat(totalData[1]) || 0;
    const apiTotal = parseFloat(totalData[2]) || 0;
    
    // 如果 API 返回的支出為 0 但我們計算出有支出，使用計算的值
    if (apiExpense === 0 && calculatedExpense > 0) {
      console.warn('[預算表] API 返回的支出總計為 0，但計算出有支出，使用計算的總計', {
      ...
      finalTotal = calculatedTotalData;
    } else if (Math.abs(apiExpense - calculatedExpense) > 0.01) {
      // 如果差異很大，記錄警告但使用 API 的值（可能是因為快取或其他原因）
      console.warn('[預算表] API 返回的支出總計與計算值有差異', {
        api: { income: apiIncome, expense: apiExpense, total: apiTotal },
        calculated: { income: calculatedIncome, expense: calculatedExpense, total: calculatedTotal },
        difference: Math.abs(apiExpense - calculatedExpense)
      });
      // 如果計算的支出明顯更大，使用計算的值
      if (calculatedExpense > apiExpense * 1.1) {
        console.warn('[預算表] 計算的支出明顯大於 API 返回的值，使用計算的總計');
        finalTotal = calculatedTotalData;
      } else {
