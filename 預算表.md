...
const base = "https://script.google.com/macros/s/AKfycbxUlj6fC-NYTZZm0YTb9ocOP6xBUxGQzHXa8rTQGYFbPgzBmFtUrjCfus9kPOVBN7Yp/exec";
...
const saveData = async () => {
  ...
  if (recordNumberElement) {
    const numberText = recordNumberElement.textContent.trim();
    const rawNumber = numberText.replace('#', '').trim();
    if (rawNumber) {
      // 將編號轉換為三位數格式
      const num = parseInt(rawNumber) || 1;
  ...
  if (!time || time === '') {
    ...
    const fallbackMonth = String(fallbackNow.getMonth() + 1).padStart(2, '0');
    const fallbackDay = String(fallbackNow.getDate()).padStart(2, '0');
    const fallbackHours = String(fallbackNow.getHours()).padStart(2, '0');
    const fallbackMinutes = String(fallbackNow.getMinutes()).padStart(2, '0');
    time = `${fallbackYear}/${fallbackMonth}/${fallbackDay} ${fallbackHours}:${fallbackMinutes}`;
  }
