...
const processDataFromResponse = (data, shouldFilter = true) => {
  ...
  // 用於合併相同項目的支出記錄：key 是 "類別_項目"，value 是合併後的記錄
  const mergedExpenseRecords = new Map();
    ...
    Object.keys(data).forEach(key => {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        if (isValidNumber) {
          ...
          // 如果是支出類型，需要合併相同項目的記錄（category + item 相同時，金額相加）
          if (type === '支出' && row.length >= 6) {
            const category = row[2] || ''; // category 在索引 2
            const item = row[3] || ''; // item 在索引 3
            const cost = parseFloat(row[4] || 0) || 0; // cost 在索引 4
            const mergeKey = `${category}_${item}`;
            
            if (mergedExpenseRecords.has(mergeKey)) {
              // 如果已經存在相同項目的記錄，合併金額
              const existingRecord = mergedExpenseRecords.get(mergeKey);
              const existingCost = parseFloat(existingRecord.row[4] || 0) || 0;
              const mergedCost = existingCost + cost;
              
              // 更新金額（保留第一個編號和時間，金額相加）
              existingRecord.row[4] = mergedCost;
              
              console.log('[預算表] processDataFromResponse: 合併相同項目的支出記錄', {
                mergeKey: mergeKey,
                category: category,
                item: item,
              ...
              // 如果不存在，添加到合併記錄中
              mergedExpenseRecords.set(mergeKey, { type, row: [...row] });
            }
          } else {
            // 收入類型或格式不正確的支出，直接添加
...
const updateTotalDisplay = (totalData = null) => {
  ...
  if (false && totalData && Array.isArray(totalData) && totalData.length >= 3) {
    ...
    // 重要：初始顯示時，costInput.value 與記錄中的值相同，不應該被視為正在修改
    ...
    if (!isNewMode && filteredRecords.length > 0 && currentRecordIndex >= 0 && currentRecordIndex < filteredRecords.length) {
      ...
      // 檢查用戶是否正在修改金額
      // 重要：只有在 costInput.value 與記錄中的值不同時，才視為正在修改
      // 初始顯示時，costInput.value 應該與記錄中的值相同，所以 isEditingAmount 應該是 false
      if (costInput && costInput.value !== undefined && costInput.value !== null && costInput.value !== '') {
        ...
        // 使用較小的閾值（0.01）來判斷是否不同，避免浮點數誤差
        if (Math.abs(newAmount - oldAmount) > 0.01) {
          isEditingAmount = true;
          console.log('[預算表] updateTotalDisplay: 檢測到用戶正在修改金額', {
            oldAmount: oldAmount,
            newAmount: newAmount,
            difference: Math.abs(newAmount - oldAmount)
          });
        } else {
          // 金額相同，表示沒有修改（可能是初始顯示）
          isEditingAmount = false;
          console.log('[預算表] updateTotalDisplay: 金額相同，視為初始顯示', {
            oldAmount: oldAmount,
            newAmount: newAmount
          });
        }
      } else {
        // costInput.value 為空，表示沒有修改
        isEditingAmount = false;
      }
    }
  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      if (result.data && result.total) {
        ...
        // 根據當前類型重新過濾記錄（在更新總計之前）
        const currentType = categorySelect ? categorySelect.value : recordType;
        filterRecordsByType(currentType);

        // 等待過濾完成，確保 allRecords 和 filteredRecords 都已更新
        await new Promise(r => setTimeout(r, 100));

        // 立即更新總計（在更新資料和過濾後立即更新）
        // 重要：刪除記錄後，應該從 allRecords 重新計算總計，確保總計正確
        console.log('[預算表] deleteCurrentRecord: 刪除後從 allRecords 重新計算總計', {
          allRecordsCount: allRecords.length,
          filteredRecordsCount: filteredRecords.length,
          resultTotal: result.total
        });
        updateTotalDisplay(null); // 傳入 null，強制從 allRecords 計算總計
        
        // 更新記憶體中的總計（使用計算的結果）
        // 注意：我們從 allRecords 計算總計，而不是使用後端返回的總計，以確保一致性
        if (result.total && Array.isArray(result.total) && result.total.length >= 3) {
          // 更新記憶體中的總計（使用後端返回的作為參考，但顯示使用計算的）
