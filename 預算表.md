...
const base = "https://script.google.com/macros/s/AKfycbw2h9QZ0KodtOVpeltaMnM1xZu53Thx6ZqSz6MnwItQEK4PAT7TiJYLlIOzhrZM8hANIA/exec";
// 目前選擇的試算表分頁索引（對應 Apps Script 中的 getSheets()[index]，2 代表第三個分頁）
let currentSheetIndex = 2;
// 從 Show Tab Name 取得的所有月份分頁名稱（不含前兩個「空白表」、「下拉選單」）
let sheetNames = [];

// 預先載入的所有月份資料（key: sheetIndex, value: { data: {...}, total: [...] }）
let allMonthsData = {}; // 儲存每個月份的資料和總計

// 從試算表載入的所有記錄（收入 + 支出），依序存起來
let allRecords = []; // 目前選擇月份的記錄
let filteredRecords = []; // 目前類型過濾後的記錄
let currentRecordIndex = 0;
let isNewMode = false; // 是否在新增模式
let currentRecordNumber = null; // 目前顯示的記錄編號（用於切換類型時保持相同編號）
...
const processDataFromResponse = (data, shouldFilter = true) => {
  ...
  if (data && typeof data === 'object') {
    // 獲取當前月份名稱
    const currentMonthName = sheetNames[currentSheetIndex - 2] || '';
    ...
    Object.keys(data).forEach(key => {
      ...
      if (!isCurrentMonth && (isIncome || isExpense)) {
        console.warn(`[processDataFromResponse] 跳過非當前月份的資料: ${key}`);
        return;
      }
...
// 初始化月份下拉選單：從 Google 試算表取得分頁名稱，並根據選擇載入對應月份資料
const initMonthSelect = async () => {
  if (!monthSelect) return;
  try {
    const params = { name: "Show Tab Name" };
    const namesUrl = `${base}?${new URLSearchParams(params)}`;
    ...
    // 建立選項：ShowTabName 已經排除了前兩個分頁（空白表、下拉選單）
    sheetNames.forEach((name, idx) => {
      const opt = document.createElement('option');
      opt.value = String(idx); // 0 對應 getSheets()[2]，1 對應 getSheets()[3] ...
      opt.textContent = name;
      monthSelect.appendChild(opt);
    });
    
    // 預設選擇最後一個月份分頁
    const defaultIndex = sheetNames.length - 1;
    monthSelect.value = String(defaultIndex);
    currentSheetIndex = defaultIndex + 2; // 轉為實際 sheet index
