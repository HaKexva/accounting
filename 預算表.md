...
// Budget
const baseBudget = "https://script.google.com/macros/s/AKfycbxdF4weI0EoE3ug_Dxrve-gteLGrf9g9rJQ5Zh11ovZ6FscOZUu7Xbr0RIGV74DSLaa/exec";
...
// API URL（支出表）
const baseExpense = "https://script.google.com/macros/s/AKfycbwMELdOAUBfZ7cYx_gakzBlu0E0uqrrDSOJiHI8oZSq-y4yijnJ9llJhd6Gjs6zW4kw/exec";
...
const processDataFromResponse = (data, shouldFilter = true) => {
  ...
  console.log('[預算表] processDataFromResponse: currentSheetIndex =', currentSheetIndex, 'monthIndex =', monthIndex, 'currentMonthName =', currentMonthName, 'sheetNames =', sheetNames);
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    console.log('[預算表] processDataFromResponse: 資料是物件格式，keys =', Object.keys(data));

    // 處理物件格式的資料（命名範圍格式）
    Object.keys(data).forEach(key => {
      const rows = data[key] || [];
      console.log('[預算表] 處理 key:', key, 'rows.length =', rows.length);
      ...
      if (!type) {
        console.log('[預算表] key', key, '不是收入或支出，跳過');
        return;
      }
      ...
      if (!currentMonthName || currentMonthName === '') {
        ...
        isCurrentMonth = true;
        console.log('[預算表] 沒有月份名稱，處理所有符合類型的資料，key =', key);
      } else {
        // 檢查 key 是否包含當前月份名稱
      ...
      console.log('[預算表] key', key, 'isCurrentMonth =', isCurrentMonth, 'currentMonthName =', currentMonthName, 'type =', type);
      if (!isCurrentMonth) {
        console.log('[預算表] key', key, '不是當前月份，跳過');
        return;
      }
    ...
    console.log('[預算表] processDataFromResponse: 處理完成，共', processedCount, '筆記錄，allRecords.length =', allRecords.length);
...
const updateTotalDisplay = (totalData = null) => {
  console.log('[預算表] updateTotalDisplay: 收到 totalData =', totalData);
  let income = 0;
  let expense = 0;
  let total = 0;
  ...
  if (totalData && Array.isArray(totalData) && totalData.length >= 3) {
    ...
    console.log('[預算表] updateTotalDisplay: 解析後 income =', income, 'expense =', expense, 'total =', total);
  } else {
    // 從當前記錄計算即時總計
    console.log('[預算表] updateTotalDisplay: 使用即時計算，allRecords.length =', allRecords.length);
    const incomeRecords = allRecords.filter(r => r.type === '收入');
    const expenseRecords = allRecords.filter(r => r.type === '支出');

    console.log('[預算表] updateTotalDisplay: incomeRecords.length =', incomeRecords.length, 'expenseRecords.length =', expenseRecords.length);

    // 計算收入總計
    income = incomeRecords.reduce((sum, r) => {
      if (!r || !r.row || !Array.isArray(r.row)) return sum;
      const cost = parseFloat(r.row[3] || 0) || 0; // 收入：row[3] 是金額
      console.log('[預算表] updateTotalDisplay: 收入記錄，row[3] =', r.row[3], 'cost =', cost);
      return sum + cost;
    }, 0);
    ...
    expense = expenseRecords.reduce((sum, r) => {
      ...
      console.log('[預算表] updateTotalDisplay: 支出記錄，row[4] =', r.row[4], 'cost =', cost);
      return sum + cost;
    }, 0);
    ...
    if (costInput && costInput.value) {
      ...
      console.log('[預算表] updateTotalDisplay: 即時輸入，type =', currentType, 'cost =', liveCost);
      if (currentType === '收入') {
        income += liveCost;
      } else {
        expense += liveCost;
    ...
    console.log('[預算表] updateTotalDisplay: 計算結果，income =', income, 'expense =', expense, 'total =', total);
  }
...
const loadMonthData = async (sheetIndex, useGlobalAbortController = true) => {
  ...
  console.log('[預算表] loadMonthData: 返回 totalData =', totalData);
...
const loadContentFromMemory = () => {
  ...
  // 處理資料（會自動過濾並顯示記錄）
  if (monthData.data) {
    console.log('[預算表] loadContentFromMemory: 開始處理資料，monthData.data type =', typeof monthData.data, 'isArray =', Array.isArray(monthData.data), 'sheetNames.length =', sheetNames.length);
  processDataFromResponse(monthData.data, true);
    console.log('[預算表] loadContentFromMemory: 處理完成，allRecords.length =', allRecords.length, 'filteredRecords.length =', filteredRecords.length);

    // 處理資料後立即更新總計（確保總計自動計算）
    console.log('[預算表] loadContentFromMemory: monthData.total =', monthData.total, 'isArray =', Array.isArray(monthData.total));
    if (monthData.total && Array.isArray(monthData.total) && monthData.total.length >= 3) {
      console.log('[預算表] loadContentFromMemory: 使用儲存的總計', monthData.total);
      updateTotalDisplay(monthData.total);
    } else {
      // 如果沒有總計或格式不正確，使用即時計算
      console.log('[預算表] loadContentFromMemory: 使用即時計算總計');
      updateTotalDisplay();
    }
  } else {
...
const loadContent = async (forceReload = false) => {
  ...
  try {
    ...
    // 處理資料（會自動過濾並顯示記錄）
    if (monthData.data) {
      console.log('[預算表] loadContent: 開始處理資料，monthData.data type =', typeof monthData.data, 'isArray =', Array.isArray(monthData.data), 'sheetNames.length =', sheetNames.length);
      processDataFromResponse(monthData.data, true);
      console.log('[預算表] loadContent: 處理完成，allRecords.length =', allRecords.length, 'filteredRecords.length =', filteredRecords.length);

      // 處理資料後立即更新總計（確保總計自動計算）
      console.log('[預算表] loadContent: monthData.total =', monthData.total, 'isArray =', Array.isArray(monthData.total));
      if (monthData.total && Array.isArray(monthData.total) && monthData.total.length >= 3) {
        console.log('[預算表] loadContent: 使用 API 總計', monthData.total);
    updateTotalDisplay(monthData.total);
      } else {
        // 如果沒有總計或格式不正確，使用即時計算
        console.log('[預算表] loadContent: 使用即時計算總計');
        updateTotalDisplay();
      }
    } else {
  ...
  try {
    ...
    if (targetSheetIndex && targetMonthName) {
      ...
      if (targetMonthData.data) {
        console.log('[預算表] initMonthSelect: 開始處理資料，sheetNames.length =', sheetNames.length);
        processDataFromResponse(targetMonthData.data, true);
        console.log('[預算表] initMonthSelect: 處理完成，allRecords.length =', allRecords.length, 'filteredRecords.length =', filteredRecords.length);
      ...
      updateTotalDisplay(targetMonthData.total);
          ...
          // 如果沒有總計或格式不正確，使用即時計算
          console.log('[預算表] initMonthSelect: 使用即時計算總計');
          updateTotalDisplay();
        }
      } else {
