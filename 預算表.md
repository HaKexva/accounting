...
// 預先載入其他月份的資料（在背景併發請求，避免之後切換月份卡頓）
const preloadAllMonthsData = async () => {
  ...
  console.log('[預載] 需要載入的月份數:', totalMonths, '月份列表:', monthsToLoad.map((name, idx) => `${name}(${idx + 2})`).join(', '));
  
  if (totalMonths === 0) {
    console.log('[預載] 沒有需要預載的月份，直接返回');
    return; // 如果沒有需要預載的月份，直接返回
  }
  ...
  // 使用並發載入以提高速度，並略過目前正在顯示的分頁（避免重複請求）
  const promises = sheetNames.map(async (name, idx) => {
    const sheetIndex = idx + 2; // 轉為實際 sheet index
    if (sheetIndex === currentSheetIndex) {
      console.log(`[預載] 跳過當前月份 ${name} (sheetIndex: ${sheetIndex})`);
      return null; // 跳過當前月份，因為已經載入過了
    }
    
    console.log(`[預載] 開始載入月份 ${name} (sheetIndex: ${sheetIndex})`);
    try {
      // 使用獨立的 AbortController，避免與用戶操作衝突
      const monthData = await loadMonthData(sheetIndex, false);
      allMonthsData[sheetIndex] = monthData;
      
      console.log(`[預載] 成功載入月份 ${name} (sheetIndex: ${sheetIndex})`);
      console.log(`[預載] ${name} 資料 keys:`, Object.keys(monthData.data || {}));
      console.log(`[預載] ${name} 總計:`, monthData.total);
      
      // 調試：檢查 202506 月份的載入結果
      if (name === '202506') {
        console.log(`[202506 調試] 成功載入，資料 keys:`, Object.keys(monthData.data || {}));
        console.log(`[202506 調試] 總計:`, monthData.total);
        Object.keys(monthData.data || {}).forEach(key => {
          const rows = monthData.data[key] || [];
          console.log(`[202506 調試] ${key}: ${rows.length} 筆資料`);
        });
      }
      ...
      // 輸出錯誤訊息以便除錯
      console.error(`[預載] 載入月份 ${name} (sheetIndex: ${sheetIndex}) 失敗:`, error);
      if (name === '202506') {
        console.error(`[202506 調試] 詳細錯誤:`, error);
        console.error(`[202506 調試] 錯誤堆疊:`, error.stack);
      }
  ...
  // 檢查是否有載入失敗的月份
  const failedMonths = results.filter(r => r && !r.success);
  if (failedMonths.length > 0) {
    console.warn('[預載] 以下月份載入失敗:', failedMonths.map(r => `${r.name} (${r.error})`).join(', '));
  }
...
const initMonthSelect = async () => {
  ...
  try {
    ...
    try {
      ...
      console.log(`[初始化] 開始載入當前月份 (sheetIndex: ${currentSheetIndex})`);
      const currentMonthData = await loadMonthData(currentSheetIndex);
      allMonthsData[currentSheetIndex] = currentMonthData;
      
      console.log(`[初始化] 當前月份載入完成，資料 keys:`, Object.keys(currentMonthData.data || {}));
      console.log(`[初始化] 當前月份總計:`, currentMonthData.total);
      ...
      // 不立即關閉載入遮罩，等待背景預載完成
      // 在背景預載其他月份的資料，並更新進度條
      console.log('[初始化] 開始預載其他月份資料');
      preloadAllMonthsData().then(() => {
    ...
    monthSelectChangeHandler = async () => {
      ...
      try {
        ...
        if (loadContentFromMemory()) {
          ...
          // 記憶體中沒有資料時，發送請求載入，但「不顯示整頁載入中遮罩」
          // 只暫時鎖定輸入與按鈕，避免在載入期間誤操作
          if (itemInput) itemInput.disabled = true;
          if (costInput) costInput.disabled = true;
          if (noteInput) noteInput.disabled = true;
          if (categorySelect) categorySelect.disabled = true;
