...
const processDataFromResponse = (data, shouldFilter = true) => {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    ...
    Object.keys(data).forEach(key => {
      ...
      // 但如果月份名稱不存在或為空，則處理所有符合類型的資料（用於初始化或錯誤恢復）
      const isCurrentMonth = !currentMonthName || (currentMonthName && key.includes(currentMonthName));
      if (!isCurrentMonth) {
        return;
      }
    ...
    if (response.ok && result.success) {
      ...
      if (result.data && result.total) {
        ...
        if (result.total && Array.isArray(result.total) && result.total.length >= 3) {
          ...
          // 如果後端沒有返回總計或格式不正確，重新載入總計
          await loadTotal();
          // 如果 loadTotal 也失敗，使用即時計算
          if (!allMonthsData[currentSheetIndex] || !allMonthsData[currentSheetIndex].total) {
            updateTotalDisplay(); // 不傳參數，使用即時計算
          }
        }
